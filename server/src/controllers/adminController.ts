...
export const removeMemberAdminController = async (req: Request, res: Response) => {
   try {
      ...
      if (!Number.isInteger(apartmentId) || apartmentId <= 0 || !Number.isInteger(userId) || userId <= 0) {
...
export const addMemberAdminController = async (req: Request, res: Response) => {
   try {
      ...
      return res.status(201).json({ message: 'Member added. An account can only be in one apartment; they were removed from any other.', members });
   } catch (error) {
      console.error(error);
      return serverError(res);
   }
   ...
   try {
      ...
      if (refs.payments > 0 || refs.loans > 0 || refs.complaints > 0) {
         return res.status(400).json({
            message: 'Cannot delete user: they have payments, loans or complaints. Remove or reassign those first.',
            refs,
         });
      }
